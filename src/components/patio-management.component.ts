    ...
    <div class="space-y-6 animate-fade-in-up">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-slate-500 dark:text-slate-400 mt-1">
            Cadastre e gerencie os pátios da empresa
          </p>
        </div>
        <button 
          ...
          <form (ngSubmit)="savePatio()" class="space-y-4">
            ...
            <div class="flex items-center gap-4 pt-4">
              <label class="flex items-center gap-2 cursor-pointer">
                ...
                <span class="text-sm text-slate-700 dark:text-slate-300">Pátio Ativo</span>
              </label>
            </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @for (patio of dataService.patios(); track patio.id) {
          <div class="glass-card p-6 rounded-2xl hover:scale-[1.02] transition-transform duration-300" [class.opacity-50]="!patio.isActive">
            <div class="flex items-start justify-between mb-4">
              <div class="flex-1">
                <h3 class="text-xl font-bold text-slate-900 dark:text-white">{{ patio.name }}</h3>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ patio.city }}, {{ patio.state }}</p>
              </div>
              <div class="flex items-center gap-2">
                @if (patio.isActive) {
                  <span class="px-2 py-1 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 text-xs font-bold">
                    Ativo
                  </span>
                } @else {
                  <span class="px-2 py-1 rounded-lg bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-400 text-xs font-bold">
                    Inativo
                  </span>
                }
              </div>
            ...
            <div class="space-y-2 mb-4">
              ...
              <div class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                ...
                Criado em {{ patio.createdAt | date:'dd/MM/yyyy' }}
              </div>
            ...
            <div class="flex gap-2 pt-4 border-t border-slate-200 dark:border-slate-700">
              <button 
                ...
                class="flex-1 px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-medium hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors text-sm">
                Editar
              </button>
              <button 
                ...
                class="flex-1 px-4 py-2 rounded-lg font-medium hover:opacity-80 transition-opacity text-sm">
